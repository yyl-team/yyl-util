  ...
  runCMD: function(str, callback, path, showOutput, newWindow) {
    ...
    if (newWindow) {
      if (IS_WINDOWS) {
        ...
        iCmd = `osascript -e 'tell application "Terminal" to activate' -e 'tell application "System Events" to tell process "Terminal" to keystroke "t" using command down' -e 'tell application "Terminal" to do script "cd ${path} && ${iCmd}" in selected tab of the front window'`;
      }
    }
  ...
  infoBar: {
    ...
    print: function(type, ctx, ctx2) {
      ...
      if (util.type(op.barLeft) == 'array') {
        op.barLeft.forEach((barLeft, i) => {
          let headStr;
          let footStr = '';
          padding = process.stdout.columns;
          if (i === 0) {
            headStr = buildHeader(headInfo);
          } else {
            headStr = buildHeader(util.extend({}, headInfo, {
              name: ''
            }));
          }
          if (op.foot) {
          ...
          const barStr = buildBar(util.extend({}, op, {
            barLeft: barLeft
          }));
          ...
          process.stdout.write(`${headStr}${barStr}${footStr}${space}`);
        });
      } else {
