  ...
  runCMD: function(str, callback, path, showOutput, newWindow) {
    ...
    if (newWindow) {
      if (IS_WINDOWS) {
        ...
        iCmd = `osascript -e 'tell application "Terminal" to activate' -e 'tell application "System Events" to tell process "Terminal" to keystroke "t" using command down' -e 'tell application "Terminal" to do script "${iCmd}" in selected tab of the front window'`;
      }
    }
