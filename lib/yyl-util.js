                    ...
                    for(var key in arr){
                        if(arr.hasOwnProperty(key)){
                            if(util.type(arr[key]) == 'array'){
                                r.push( chalk.gray(textIndent(key, baseIndent * 2)) + textIndent(arr[key].shift(), maxkeyLen - key.length + 2));
                                ...
                                r.push(chalk.gray(textIndent(key, baseIndent * 2)) + textIndent(arr[key], maxkeyLen - key.length + 2));
        ...
        msg: {
            source: {
                type: {
                    ...
                    info: '#999999',
                    del:{name: 'deleted', color: '#e5004f'}
                },
                maxSize: 7
            },
            init: function(type){
                var self = this;
                util.extend(true, self.source.type, type);
