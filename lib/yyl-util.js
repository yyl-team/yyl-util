...
const util = {
   ...
   * @return {Void}
   */
  runCMD: function(str, callback, path, showOutput, newWindow) {
  ...
  readFilesSync: function(iPath, filter, ignoreFilter) {
    const r = [];
    const deep = function(rPath) {
      ...
      list.forEach((str) => {
        ...
        const filterPath = util.path.relative(iPath, mPath);
  ...
  msg: {
    ...
    replace: function() {
      ...
      if (str.length > COLUMNS) {
        str = `${str.substr(0, 25)}...${  str.substr(str.length - COLUMNS - 3)}`;
      }
  ...
  infoBar: {
    ...
    init: function(op) {
      ...
      Object.keys(options.head.key).forEach((key) => {
        ...
        if (util.getStrSize(options.head.key[key].name) > options.head.size) {
          options.head.size = util.getStrSize(options.head.key[key].name);
        }
        self[key] = function() {
    ...
    print: function(type, ctx, ctx2) {
      ...
      // build headbar
      const buildHeader = (info, noPadding) => {
        if (!noPadding) {
        ...
        return co(` ${info.name + fn.makeSpace(options.head.size - util.getStrSize(info.name))} `);
      };
      const buildFooter = (info, noPadding) => {
        if (!noPadding) {
          padding = padding - options.foot.size - 2;
        }

        if (util.getStrSize(info.ctx) > options.foot.size) {
          info.ctx = `${util.substr(info.ctx, 0, options.foot.size - 3)}...`;
        }
        ...
        return co(` ${fn.makeSpace(options.foot.size - util.getStrSize(info.ctx)) + info.ctx}`);
      };
      const buildBar = (info) => {
        const runHl = she.__fn.hightlighter;
        const leftLen = util.getStrSize(info.barLeft);
        const rightLen = util.getStrSize(info.barRight);
        if (leftLen + rightLen > padding - 3) {
          let overSize = leftLen + rightLen - padding + 3;
          if (!info.foot) {
            overSize -= 1;
          }
          if (leftLen > overSize) {
            info.barLeft = `${util.substr(info.barLeft, 0, leftLen - overSize - 3)}...`;
          } else if (rightLen > overSize) {
            info.barRight = `...${util.substr(info.barRight, overSize + 3)}`;
          } else {
            info.barRight = '';
            if (rightLen > padding - 2) {
              info.barLeft = `${util.substr(info.barLeft, 0, padding - 5)}...`;
            }
          }
        }
        const spaceLen = padding -
          util.getStrSize(info.barLeft) -
          util.getStrSize(info.barRight) - 2;
        return ` ${runHl(info.barLeft)}${fn.makeSpace(spaceLen)}${runHl(info.barRight)} `;
      };



      if (util.type(op.barLeft) == 'array') {
        let footStr = '';
        let subFootStr = '';
        let rightStr = '';
        let subRightStr = '';
        const headStr = buildHeader(headInfo);
        const subHeadStr = buildHeader(util.extend({}, headInfo, {
          name: ''
        }), true);
        if (op.foot) {
          footStr = `${buildFooter(util.extend({}, options.foot, {ctx: op.foot}))} `;
          subFootStr = `${buildFooter(util.extend({}, options.foot, {ctx: ''}), true)} `;
        }
        if (op.barRight) {
          rightStr = ` ${op.barRight}`;
          subRightStr = ` ${fn.makeSpace(util.getStrSize(op.barRight))}`;
          padding -= util.getStrSize(rightStr);
        }

        padding -= 1;
        // 数据处理
        const leftArr = [];
        op.barLeft.forEach((barLeft) => {
          barLeft.split(/[\r\n]+/).forEach((leftstr) => {
            if (!leftstr) {
              return;
            }
            if (util.getStrSize(leftstr) <= padding) { // 单行
              leftArr.push(leftstr);
            } else { // 多行
              let rStr = leftstr;
              let isFirst = true;
              while (rStr) {
                if (isFirst) {
                  leftArr.push(util.substr(rStr, 0, padding));
                  rStr = util.substr(rStr, padding);
                  isFirst = false;
                } else {
                  if (util.getStrSize(rStr) <= padding - 2) {
                    leftArr.push(`  ${rStr}`);
                    rStr = '';
                  } else {
                    leftArr.push(`  ${util.substr(rStr, 0, padding - 2)}`);
                    rStr = util.substr(rStr, padding - 2);
                  }
                }
              }
            }
        ...
        leftArr.forEach((barLeft, i) => {
          const space = fn.makeSpace(padding - util.getStrSize(barLeft));
          if (i == 0) {
            process.stdout.write(`${headStr} ${barLeft}${space}${rightStr}${footStr}`);
          } else {
            process.stdout.write(`${subHeadStr} ${barLeft}${space}${subRightStr}${subFootStr}`);
          }
        });
      } else {
        ...
        let footStr = '';
        if (op.foot) {
          footStr = buildFooter(util.extend({}, options.foot, {
            ctx: op.foot
          }));
        }
  ...
  substr: (str, begin, len) => {
    ...
    const size = util.getStrSize(str);
